...
- **Web 播放器旋转 / 全屏 / 自动起播**（`2026-09-17`）：
  - 右上角新增画面旋转按钮（`player-tools` 工具组），按 0°→90°→180°→270° 循环；90/270
    时按容器与视频实际比例等比缩放，避免旋转后溢出被裁。**已知限制**：用的是原生
    `<video controls>`，控件随画面一起旋转，要做到控件不跟着转需要自绘控件。
  - 全屏改由整个 `.playback-view` 容器承担：原生按钮只会把 `<video>` 全屏，导致右上角
    轨道/旋转控件消失；现在检测到 video 独占全屏会改请求容器全屏。
